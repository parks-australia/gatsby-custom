...
namespace Drupal\gatsby_extras\Plugin\jsonapi\FieldEnhancer;
    ...
    if (isset($data['uri'])) {
      ...
      if (!empty($parsed_uri)) {
        ...
        if (!is_null($entity)) {
          ...
          $data['uri_alias'] = \Drupal::service('path_alias.manager')->getAliasByPath('/node/' . $entity->id());
        }
        // Remove the value.
        else {
          $data = [
    ...
    if (isset($value['uri'])) {
      ...
      if (!empty($parsed_uri)) {
        ...
        if (!empty($entities)) {
          ...
          $value['uri_alias'] = \Drupal::service('path_alias.manager')->getAliasByPath('/node/' . $entity->id());
        }
        else {
